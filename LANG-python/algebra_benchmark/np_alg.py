...
class mtrx3Np(object):
    __slots__ = ["data"]
    
    def __init__(self) -> None:
        self.data = np.array([x-x for x in range(9)], dtype = np.float32)
...
def mtrx3NpFromEuler(yaw: float, pitch: float, roll: float) -> mtrx3Np:
    cosy = cos(yaw)
    siny = sin(yaw)
    cosp = cos(pitch)
    sinp = sin(pitch)
...
class CVec3Np(object):
    ...
    def __init__(self, arr: list) -> None:
        ...
        self.data = np.array([elem for elem in arr], dtype = np.float32)
        self.capacity = len(arr)
        self.dataSize = len(arr)*4 # size of array.array("f", [...]) element is 4 bytes 

    def applyMtrx3Naive(self, A: mtrx3Np) -> None:
    ...
    def applyMtrx3(self, A: mtrx3Np) -> None:
        for arId in range(0, self.capacity, 3):
            x = self.data[arId+0]
            y = self.data[arId+1]
            z = self.data[arId+2]
    ...
    def applyMtrx3TmpVec(self, A: mtrx3Np) -> None:
