...
func main() {
	pStrFlag = flag.String("str", "", "Enter string to search within")
	pSubstrFlag = flag.String("substr", "", "Enter substring to search for")
	flag.Parse()

	if SearchString(*pStrFlag, *pSubstrFlag) == -1 {
		fmt.Println("false")
	} else {
		fmt.Println("true")
	}
